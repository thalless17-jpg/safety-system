...
<div id="login-screen">
  <div class="ll-wrap">
    ...
    <div class="ll-card">
      ...
      <select id="login-loc-sel" style="display:none"><option value="">-- All locations --</option></select>
...
function save() {
  ...
  if (D.cfg?.syncBackend) {
    pushToRemote(true).catch(e => console.warn('Auto-push failed:', e.message));
  }
...
function renderSettings(){
  ...
  const fbUrl2=document.getElementById('cfg-firebase-url');if(fbUrl2)fbUrl2.value=D.cfg.firebaseUrl||'';
  const fbKey2=document.getElementById('cfg-firebase-key');if(fbKey2)fbKey2.value=D.cfg.firebaseKey||'';
}
...
function saveCfg(){
  ...
  const fbUrl=document.getElementById('cfg-firebase-url');if(fbUrl)D.cfg.firebaseUrl=fbUrl.value.trim();
  const fbKey=document.getElementById('cfg-firebase-key');if(fbKey)D.cfg.firebaseKey=fbKey.value.trim();
  logActivity('EDIT','settings','System settings',`Updated by ${SESSION?.display||'Admin'}`);
  save();updateSessionUI();
  if(D.cfg.syncBackend){startRemoteSync();}
}
...
// Firebase stores [{a:1},{b:2}] as {"0":{a:1},"1":{b:2}}
function firebaseToArray(val) {
  if (!val) return [];
  if (Array.isArray(val)) return val;
  if (typeof val === 'object') {
    const keys = Object.keys(val);
    // Check if all keys are numeric strings — Firebase array pattern
    const isArr = keys.length > 0 && keys.every(k => /^[0-9]+$/.test(k));
    if (isArr) {
      const arr = [];
...
function _mergeRemoteData(remote) {
  ...
  // Convert all Firebase object-arrays back to real JS arrays
  const arrayFields = ['employees','locations','users','ppeItems','deliveries',
    'incidents','evaluations','warnings','trainings','purchaseOrders',
    'corrActions','activityLog','deleteRequests','logEntries','autoActions'];
  arrayFields.forEach(k => { D[k] = firebaseToArray(D[k]); });

  // Merge users: local password wins
  if (!D.users) D.users = [];
...
async function pullFromRemote(force) {
  ...
  try {
    ...
    // force=true (manual Pull button) always applies data regardless of timestamp
    if (remote && (force || (remote._ts||0) >= (D._ts||0))) {
      _mergeRemoteData(remote);
      renderAll();
      toast('✅ Data loaded from remote!');
    } else if (remote) {
      showSyncStatus('synced'); // already up to date
    }
    showSyncStatus('synced');
  } catch(e) {
...
async function pushToRemote(silent) {
  if (!D.cfg?.syncBackend) {
    if(!silent) toast('Configure cloud sync in Settings first', 'warn');
    return;
  }
  D._ts = Date.now();
  _saveLocal();
...
async function firebasePull() {
  ...
  const r = await fetch(url+'/.json'+auth, {headers:{'Accept':'application/json'}});
  if (r.status===401) throw new Error('Firebase: Unauthorized — add secret key in Settings');
  ...
  if (!data) return null;
  // Auto-detect: if data has 'safetyapp' node, read from there
  // Otherwise use root data directly (backup uploaded to root)
  if (data.safetyapp && typeof data.safetyapp === 'object' && data.safetyapp.users) {
    console.log('[Firebase] Reading from /safetyapp node');
    return data.safetyapp;
  }
  // Root has the data (backup uploaded directly to root)
  if (data.users || data.employees || data.locations) {
    console.log('[Firebase] Reading from root node');
    return data;
  }
  return data;
}
...
async function firebasePush() {
  ...
  if (!url) throw new Error('Firebase URL not configured in Settings');
  const auth = D.cfg.firebaseKey ? '?auth='+D.cfg.firebaseKey : '';
  const r = await fetch(url+'/.json'+auth, {
    method: 'PUT',
...
function startFirebaseSSE() {
  ...
  const auth = D.cfg.firebaseKey ? '?auth='+D.cfg.firebaseKey : '';
  try {
    _firebaseSSE = new EventSource(url+'/.json'+auth);
...
window.addEventListener('load', async () => {
  ...
  // Clear invalid sync config from old localStorage
  // Validate sync config — clear only if backend set but URL truly missing
  if(D.cfg && D.cfg.syncBackend){
    const b=D.cfg.syncBackend;
    const invalid=(b==='custom'&&!D.cfg.syncUrl)||(b==='jsonbin'&&!D.cfg.jsonbinId)||(b==='sharepoint'&&!D.cfg.spSiteUrl);
    // Note: firebase is NOT cleared here — URL may be set in Settings UI
    if(invalid){D.cfg.syncBackend='';try{localStorage.setItem(SK,JSON.stringify(D));}catch(e){}}
  }
  ...
  // Cloud sync: start if configured
  if (D.cfg?.syncBackend) {
    // Auto-pull on startup to load latest data
    pullFromRemote(true)
      .then(() => { console.log('[SafetyApp] Initial pull complete'); })
      .catch(e => console.warn('[SafetyApp] Initial pull skipped:', e.message));
    startRemoteSync();
  }
